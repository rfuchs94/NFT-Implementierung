...
// Hier wird der Befehl Upload ausgeführt aus dem TypeScript
programCommand('upload')
  .argument(
    '<directory>', // Abfrage nach dem Ordner Path wo die Bilder und Metadaten abgelegt sind
    'Directory containing images named from 0-n',
    val => {
      return fs.readdirSync(`${val}`).map(file => path.join(val, file));
    },
  ) // Abfrage der Candymachine Datei, welche konfiguriert wurde für den Mint.
  .requiredOption(
    '-cp, --config-path <string>',
    'JSON file with candy machine settings',
  ) // Hier kann die RPC (Remote Procedure Call) angegeben werden in der Arbeit wurde die RPC Mainnet Endpoint: https://ssc-dao.genesysgo.net/ verwendet
    // die RPC dient als Standard für distributed client server computing src:https://www.sciencedirect.com/topics/computer-science/remote-procedure-call
  .option(
    '-r, --rpc-url <string>',
  ...
  .option(
    ...
    myParseInt,
    5,
  ) // Mint ID für die Candymachine kann vorgegeben werden ansonsten wird sie erstellt
  .option(
    '-m, --collection-mint <string>',
    'optional collection mint ID. Will be randomly generated if not provided',
  ) // optionale Eingabe ob ein NFT als Kollektion gilt oder nicht zur Verifizierung einer Kollektion (Collection NFT / Regular NFT)
    // Verifizierung findet über "Belong to" relationship statt src:https://docs.metaplex.com/programs/token-metadata/certified-collections
  .option(
  ...
  .action(async (files: string[], options, cmd) => {
    ...
    // es werden zuerst die Keys geladen / verfiziert und erst anschliessend das Candy Programm geladen. 
    ...
    const {
      ...
      goLiveDate,
      uuid,    // In Zeile 163 wird angegeben, dass das Laden der Config Datei abgewartet werden soll bis zur Ausführung der Scripts davor.
    } = await getCandyMachineV2Config(walletKeyPair, anchorProgram, configPath);
    // Wird für Deployment auf dem Devnet verwendet
    ...
    // Wird für Deployment auf dem mainnet verwendet. Arweave dient als dezentraler Speicher.
    ...
    // vorheriger Speicher von Arweave
    if (storage === StorageType.Arweave) {
      log.warn(
        'WARNING: The "arweave" storage option will be going away soon. Please migrate to arweave-bundle or arweave-sol for mainnet.\n',
      );
    }
    // Zeile 183 bis 200 gibt weitere Speicherungsmöglichkeiten an.
    ...
    if (storage === StorageType.Aws && !awsS3Bucket) {
     ...
     // der Storage Type und der Storage müssen übereinstimmen asonsten wird eine Fehlermeldung ausgegeben. 
    ...
    // Nicht unterstützte File Typen werden gefiltert und JSON vs Bild Files gezählt als Abgleichung.
    ...
    // animation files können nicht auf arweave gespeichert werden.
    ...
    // Zeile 240-256 sind diverse if Bedingungen zur Ausgabe von Fehlermeldungen in Bezug auf Speicher, Filetype und ungleiche Anzahl vs angegebene Bilder.
    ...
    // Wenn die Anforderungen zum Speicher erfüllt wurden wird die Bestätigung ausgegeben, dass der Upload startet.
    ...
    try {
      await uploadV2({ // Programm zum Upload wird ausgeführt
