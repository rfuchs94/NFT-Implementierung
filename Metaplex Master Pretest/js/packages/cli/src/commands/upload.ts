...
export async function uploadV2({
  ...
  if (!cacheContent.program.uuid) {
    ...
    try {
      ...
      // Hier wird die Candymachine initialisiert - es werden die Daten aus der config datei als splToken ausgegeben.
  ...
  const uploadedItems = Object.values(cacheContent.items).filter(
    ...
    // wenn ein Bild hochgeladen wurde wird es hier angezeigt wieviele Bilder von wie viel total hochgeladen wurden.
